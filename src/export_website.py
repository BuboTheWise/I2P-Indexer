...
def generate_address_book_txt(db_path: str, output_dir: str) -> pathlib.Path:
    ...
    for row in raw_rows:
        ...
        # Use dest_data blob from targets if available; fall back to b32.i2p
        value_blob = dest_map.get(dns_name) or ""
        if b32:
            comment = f"#{dns_name}: {b32}.b32.i2p [{status}] probed={probed_str}"
            value = dns_name + "=" + (value_blob if value_blob else f"{b32}.b32.i2p")
        else:
            comment = f"#{dns_name}:  [{status}] probed={probed_str}"
            value = dns_name + "=" + (value_blob or "")

        host_lines.append(comment)
        host_lines.append(value)
